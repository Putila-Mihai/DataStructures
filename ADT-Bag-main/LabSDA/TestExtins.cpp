//
// Created by ultra on 28.04.2023.
//
#include <assert.h>
#include "Colectie.h"
...
void testAdauga() {
    Colectie c(230); //adaugam elemente [0, 10)
    for (int i = 0; i < 10; i++) {
        c.adauga(i);
    }
    assert(c.vida() == false);
...
void testIterator() { // nu stim reprezentarea Colectiei, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
    ...
    assert(count == 50);
    Colectie c4(90); // adaugam niste elemente in colectie
    for (int i = 0; i < 100; i++) {
        c4.adauga(i);
        c4.adauga(i * (-2));
        c4.adauga(i * 2);
    ...
    Colectie c5(90); // adaugam niste elemente in colectie
    for (int i = 0; i < 100; i++) {
        c5.adauga(i);
        c5.adauga(i * (-2));
        c5.adauga(i * 2);
